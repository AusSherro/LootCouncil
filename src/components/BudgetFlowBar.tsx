    ...
    <div className="card border-border p-5">
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-6 text-sm">
          ...
          <div className="text-right">
            <p className="text-neutral text-xs tracking-wide">Assigned</p>
            <p className="font-medium text-gold tabular-nums">{formatCurrency(assigned)}</p>
          </div>
          <ArrowRight className="w-4 h-4 text-neutral/40" />
          <div className="text-right">
            <p className="text-neutral text-xs tracking-wide">Spent</p>
            <p className={`font-medium tabular-nums ${activity < 0 ? 'text-danger' : 'text-foreground'} flex items-center gap-1 justify-end`}>
      ...
      {totalPool > 0 && (
        <div className="mb-3">
          <div className="h-4 rounded-lg overflow-hidden flex bg-background-tertiary relative group">
            {/* Healthy assigned segment */}
            {healthyAssignedPercent > 0 && (
              <div
                className="h-full bg-gold/60 transition-all duration-300 ease-out relative group/seg"
                style={{ width: `${healthyAssignedPercent}%` }}
                title={`Assigned: ${formatCurrency(assigned - overspentTotal)}`}
              >
                {healthyAssignedPercent > 15 && (
                  <span className="absolute inset-0 flex items-center justify-center text-[0.625rem] font-medium text-background/70">
                    Assigned
                  </span>
                )}
              </div>
            ...
            {displayOverspentPercent > 0 && (
              <div
                className="h-full bg-danger/60 transition-all duration-300 ease-out relative"
              ...
              >
                {displayOverspentPercent > 12 && (
                  <span className="absolute inset-0 flex items-center justify-center text-[0.625rem] font-medium text-white/70">
                    Overspent
                  </span>
                )}
              </div>
            ...
            {availablePercent > 0 && (
              <div
                className="h-full bg-positive/45 transition-all duration-300 ease-out relative"
            ...
            {isOverAssigned && (
              <div
                className="h-full bg-danger/30 transition-all duration-300 ease-out flex-shrink-0"
